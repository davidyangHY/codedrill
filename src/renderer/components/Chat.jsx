...
export default function Chat({ messages, onSend, busy, ready }) {
  ...
  // cleared — so it always collapses back to exactly one line. Measure after
  // layout settles (rAF) to avoid a stale mount-time reading.
  useEffect(() => {
  ...
  return (
    <div className="flex flex-col h-full bg-base-850">
      <div className="px-4 py-2.5 border-b border-base-600 flex items-center gap-2.5 bg-base-850">
        <span className="w-6 h-6 rounded-lg brand-grad flex items-center justify-center text-[11px] font-bold tracking-tight">
          CD
        </span>
        <div>
      ...
      <div className="border-t border-base-600 p-3 bg-base-850">
        <div className="flex items-end gap-2 bg-base-900 border border-base-600 rounded-xl px-2.5 py-1.5 focus-within:border-accent transition-colors">
          <textarea
            ref={taRef}
            rows={1}
            value={input}
            ...
            className="flex-1 block bg-transparent resize-none outline-none text-sm placeholder:text-base-500 max-h-[140px] leading-5 py-1"
          />
          <button
            onClick={submit}
            disabled={busy || !input.trim() || !ready}
            className="shrink-0 w-6 h-6 rounded-md bg-accent hover:bg-accent-hover disabled:opacity-40 disabled:cursor-not-allowed flex items-center justify-center transition-colors"
            title="Send (Enter)"
          >
            {busy ? (
              ...
              <svg width="13" height="13" viewBox="0 0 24 24" fill="none">
                <path d="M4 12h14M12 5l7 7-7 7" stroke="white" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" />
              </svg>
            )}
          </button>
        </div>
